javascript:(function() {
  ...
  htmlContent += '<script>var eleves = document.getElementsByTagName("div"); var index = 0; var show = false; var indexElevesFaciles=[]; eleves[index].style.display = "block"; var sectionsReponse = document.getElementsByTagName("section"); function montrerNomPrenom(){ sectionsReponse[index].style.display="block"; show = true}; function eleveSuivant() {show = false; do {sectionsReponse[index].style.display="none"; eleves[index].style.display = "none"; index = (index + 1) % eleves.length;} while (indexElevesFaciles.includes(index)); eleves[index].style.display = "block"; }; function difficile() {eleveSuivant()}; function facile() {indexElevesFaciles.push(index); setTimeout(function () { var positionIndex = indexElevesFaciles.indexOf(index); if (positionIndex !== -1) {indexElevesFaciles.splice(positionIndex, 1); } }, 2 * 60 * 1000); eleveSuivant()}; window.addEventListener("keydown", function (event) { if (event.keyCode === 13) {montrerNomPrenom();} if (event.keyCode === 37 && show == true) { difficile(); } if (event.keyCode === 39  && show == true) { facile(); }  });</script>';
